...
namespace Leap.Unity {
  /** Collision brushes */
  public class InteractionBrushHandConstraints : HandModel {
      ...
      for (int fingerIndex = 0; fingerIndex < N_FINGERS; fingerIndex++) {
        for (int jointIndex = 0; jointIndex < N_ACTIVE_BONES; jointIndex++) {
          ...
          if (collider) {
            ...
            collider.GetCapsulePoints(out a, out b);
            StatusDrawer.DrawSphere(a, collider.radius * 2);
            StatusDrawer.DrawLine(a, b, collider.radius * 2);
            StatusDrawer.DrawSphere(b, collider.radius * 2);
          }
