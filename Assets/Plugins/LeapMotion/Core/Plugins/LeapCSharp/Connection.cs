...
namespace LeapInternal {
  ...
  public class Connection {
    ...
    protected virtual void Dispose(bool disposing) {
      ...
      UnityEngine.Debug.Log("Dispose called.");
      Stop();
    ...
    public void Start(bool multiDeviceAware = false) {
      ...
      config.flags = multiDeviceAware ?
        (uint)eLeapConnectionFlag.eLeapConnectionFlag_MultipleDevicesAware : 0;
    ...
    public void Start(LEAP_CONNECTION_CONFIG config) {
      ...
      if (_leapConnection == IntPtr.Zero) {
        ...
        UnityEngine.Debug.Log("_leapConnection was not zero, " + 
          _leapConnection);
      }
      result = LeapC.OpenConnection(_leapConnection);
      //UnityEngine.Debug.Log("LeapC OpenConnection call was " + result);
      if (result != eLeapRS.eLeapRS_Success) {
        reportAbnormalResults("LeapC OpenConnection call was", result);
        return;
      }

      // Check the connection now that it's been opened
      LEAP_CONNECTION_INFO pInfo = new LEAP_CONNECTION_INFO();
      pInfo.size = (uint)Marshal.SizeOf(pInfo);
      result = LeapC.GetConnectionInfo(_leapConnection, ref pInfo);
      //UnityEngine.Debug.Log("HI");
      //UnityEngine.Debug.Log("LeapC GetConnectionInfo call was " + result +
      //  " and info status is " + pInfo.status);
      ...
      result = LeapC.SetAllocator(_leapConnection, ref _pLeapAllocator);
      if (result != eLeapRS.eLeapRS_Success) {
        reportAbnormalResults("LeapC SetAllocator call was", result);
        return;
      }
      ...
      // if (result != eLeapRS.eLeapRS_Success) {
      //   reportAbnormalResults("LeapC.PollConnection on concurrent thread was",
      //     result);
      // }
      ...
      try {
        ...
        while (_isRunning) {
          ...
          if (result != eLeapRS.eLeapRS_Success) {
            ...
            //UnityEngine.Debug.Log("PollConnection successful");
          }
    ...
    public void GetInterpolatedFrame(Frame toFill, Int64 time) {
      ...
      eLeapRS result = LeapC.InterpolateFrame(_leapConnection, time,
        trackingBuffer, size);
      reportAbnormalResults("LeapC get interpolated frame call was ",
        result);
      if (result == eLeapRS.eLeapRS_Success) {
        LEAP_TRACKING_EVENT tracking_evt;
    ...
    private void handlePointMappingChange(
    ...
    public bool IsServiceConnected {
      get {
        if (_leapConnection == IntPtr.Zero) {
          //UnityEngine.Debug.Log("IsServiceConnected returning false because " +
          //  "_leapConnection is " + _leapConnection);
          return false;
        }
        ...
        if (pInfo.status == eLeapConnectionStatus.Connected) {
          ...
          //UnityEngine.Debug.Log("IsServiceConnected returning false, " +
          //  "pInfo.status was " + pInfo.status);
        }
    ...
    private void reportAbnormalResults(string context, eLeapRS result) {
      if (result != eLeapRS.eLeapRS_Success &&
         result != _lastResult) {
        //UnityEngine.Debug.Log("[reportAbnormalResults] " + context +
        //  " " + result);
