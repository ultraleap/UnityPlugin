    ...
    */
    protected virtual void UpdateHandRepresentations(Dictionary<int, HandRepresentation> all_hand_reps, ModelType modelType, Frame frame) {
      for (int i = 0; i < frame.Hands.Count; i++) {
        ...
        HandRepresentation rep = null;

            if (persistentHands) {
                foreach (var handrep in all_hand_reps.Values) {
                    if ((curHand.IsLeft && handrep.RepChirality == Chirality.Left) ||
                        (curHand.IsRight && handrep.RepChirality == Chirality.Right)) {
                        rep = handrep;
                        break;
                    }
                }
                ...
                for (var it = all_hand_reps.GetEnumerator(); it.MoveNext();)
                {
                    ...
                    if (r.Value != null)
                    {
                        ...
                        else
                        {
                            ...
                            //Debug.Log("Finishing");
                            toBeDeleted = r.Value;
                            Debug.Log("This happened and needs to not!");
                        }
                    }
                }
            }
    ...
    [SerializeField] private bool persistentHands = false;
