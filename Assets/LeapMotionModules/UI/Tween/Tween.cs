  ...
  public partial struct Tween {
    ...
    public Tween AddInterpolator(IInterpolator interpolator) {
      ...
      if (_instance.interpolatorCount >= _instance.interpolators.Length) {
        var newArray = new IInterpolator[_instance.interpolators.Length * 2];
        Array.Copy(_instance.interpolators, newArray, _instance.interpolators.Length);
        _instance.interpolators = newArray;
      }
    ...
    public void Stop() {
      ...
      if (_instance.returnToPoolUponStop) {
        Release();
      }
    }
    ...
    private void throwIfInvalid() {
      if (!isValid) {
        ...
        if (_id == TweenInstance.ID_UNUSED) {
          throw new InvalidOperationException("Tween is invalid.  Make sure you use Tween.Single or Tween.Persistent to create your Tween instead of the default constructor.");
        } else {
          throw new InvalidOperationException("Tween is invalid or was recycled.  Make sure to use Tween.Persistent if you want to keep a tween around after it finishes playing.");
        }
      }
    }
  }
