...
namespace Leap.Unity.UI.Layout {
  ...
  public class AnchorableBehaviour : MonoBehaviour {
    ...
    [Tooltip("Should the object pull away from the anchor towards the user's hand when the user's hand is nearby?")]
    [DisableIf("_isInteractionBehaviourNull", isEqualTo: true)]
    public bool isAttractedByHand = true;
    [Tooltip("If the object is attracted to hands, how far should the object be allowed to pull away from its anchor"
           + "towards a nearby InteractionHand?")]
    [DisableIf("_isInteractionBehaviourNull", isEqualTo: true)]
    public float maxHoverReach = 0.1F;
    [Tooltip("This curve defines the hover reach distance as a function of distance from the hand. "
           + "The evaluated value is multiplied by maxHoverReach, and the value is clamped between 0 and 1.")]
    [DisableIf("_isInteractionBehaviourNull", isEqualTo: true)]
    ...
    private void UpdateAttractionToHand() {
      ...
      if (interactionBehaviour != null) {
        if (isAttractedByHand) {
          ...
          Vector3 targetOffsetTowardsHand = towardsHand * maxHoverReach * reachTargetAmount;
