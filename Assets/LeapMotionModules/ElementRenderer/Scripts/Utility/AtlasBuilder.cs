...
public class AtlasBuilder {
  ...
  private void packSecondaryTextures(ProgressBar progress, UVChannelFlags channel, LeapGuiTextureFeature mainFeature, Texture2D packedTexture, Rect[] packedRects, Texture2D[] packedTextures) {
    ...
    progress.Begin(nonMainFeatures.Count, "", "Copying secondary textures: ", () => {
      foreach (var secondaryFeature in nonMainFeatures) {
        ...
        progress.Begin(secondaryFeature.data.Count, "", secondaryFeature.propertyName, () => {
          for (int i = 0; i < secondaryFeature.data.Count; i++) {
            var mainTexture = mainFeature.data[i].texture;
            if (mainTexture == null) {
              progress.Step();
              continue;
            }

            var secondaryTexture = secondaryFeature.data[i].texture;
  ...
  private void prepareForPacking(LeapGuiTextureFeature feature,
                                    out Texture2D defaultTexture,
                                    out Texture2D packedTexture) {
    feature.data.Query().Select(dataObj => processTexture(dataObj.texture)).FillArray(_textureArray);
