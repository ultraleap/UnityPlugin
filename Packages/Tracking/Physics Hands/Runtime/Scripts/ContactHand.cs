...
namespace Leap.Unity.PhysicsHands
{
    public abstract class ContactHand : MonoBehaviour
    {
        ...
        internal PhysicsHandsManager physicsHandsManager => contactParent.physicsHandsManager;
        ...
        internal void GenerateHandObjects(System.Type boneType)
        {
            ...
            palmBone.gameObject.layer = contactParent.physicsHandsManager.HandsResetLayer;
            ...
            for (int fingerIndex = 0; fingerIndex < FINGERS; fingerIndex++)
            {
                ...
                for (int jointIndex = 0; jointIndex < FINGER_BONES; jointIndex++)
                {
                    ...
                    bone.gameObject.layer = contactParent.physicsHandsManager.HandsResetLayer;
        ...
        public bool IsObjectInBoneRadius(Rigidbody rigid, ContactBone bone, float extraRadius = 0f)
        {
            ...
            if (bone.IsPalm)
            {
                overlappingColliders = PhysExts.OverlapBoxNonAllocOffset(bone.palmCollider, Vector3.zero, _colliderCache, physicsHandsManager.InteractionMask, QueryTriggerInteraction.Ignore, extraRadius);
            }
            else
            {
                overlappingColliders = PhysExts.OverlapCapsuleNonAllocOffset(bone.boneCollider, Vector3.zero, _colliderCache, physicsHandsManager.InteractionMask, QueryTriggerInteraction.Ignore, extraRadius);
            }
