...
{
    [RequireComponent(typeof(PhysicsHandsManager))]
    public class GrabHelper : MonoBehaviour
    {
        public PhysicsHandsManager physicsHandsManager;

        private bool _leftGoodState { get { return physicsHandsManager.contactParent.LeftHand.InGoodState; } }
        private bool _rightGoodState { get { return physicsHandsManager.contactParent.RightHand.InGoodState; } }

        private ContactHand _leftContactHand { get { return physicsHandsManager.contactParent.LeftHand; } }
        private ContactHand _rightContactHand { get { return physicsHandsManager.contactParent.RightHand; } }
        ...
        private void OnEnable()
        {
            ...
            {
                physicsHandsManager.OnPrePhysicsUpdate -= OnPrePhysicsUpdate;
                physicsHandsManager.OnPrePhysicsUpdate += OnPrePhysicsUpdate;
                physicsHandsManager.OnFixedFrame -= OnFixedFrame;
                physicsHandsManager.OnFixedFrame += OnFixedFrame;
            }
        }
        ...
        private void HelperOverlaps(ContactHand hand)
        {
            ...
            int results = Physics.OverlapCapsuleNonAlloc(hand.palmBone.transform.position + (-hand.palmBone.transform.up * 0.025f) + ((hand.handedness == Chirality.Left ? hand.palmBone.transform.right : -hand.palmBone.transform.right) * 0.015f),
                ...
                radiusAmount + (physicsHandsManager.HoverDistance * 2f), _colliderCache, physicsHandsManager.InteractionMask);
        ...
        private void OnValidate()
        {
            if (physicsHandsManager == null)
            {
                physicsHandsManager = GetComponent<PhysicsHandsManager>();
            }
        }
    }
}