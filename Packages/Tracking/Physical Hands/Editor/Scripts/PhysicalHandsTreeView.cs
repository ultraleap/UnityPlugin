...
public class PhysicalHandsTreeView : TreeView
{
    ...
    public PhysicalHandsTreeView(TreeViewState state, MultiColumnHeader multiColumnHeader) : base(state, multiColumnHeader)
    {
        PhysicalHandsSettings.Instance.RefreshRecommendedSettingsValues();
    ...
    protected override TreeViewItem BuildRoot()
    {
        ...
        var recommendedSettings = PhysicalHandsSettings.Instance.recommendedSettings;

        string[] ids = recommendedSettings.Keys.OrderBy(id => PhysicalHandsSettings.Instance.IsRecommendedSettingApplied(id)).ThenBy(id => recommendedSettings[id].ignored).ThenBy(id => recommendedSettings[id].property.displayName).ToArray();

        for (int i = 0; i < ids.Length; i++)
        {
            PhysicalHandsSettings.RecommendedSetting recommendedSetting;
            if (!recommendedSettings.TryGetValue(ids[i], out recommendedSetting)) continue;

            bool settingApplied = PhysicalHandsSettings.Instance.IsRecommendedSettingApplied(ids[i]);

            if (settingApplied && !PhysicalHandsSettings.Instance.showAppliedSettings) continue;
            if (recommendedSetting.ignored && !PhysicalHandsSettings.Instance.showIgnoredSettings) continue;
    ...
    private void DrawCellGUI(Rect cellRect, TreeViewItem item, Columns column, ref RowGUIArgs args)
    {
        ...
        PhysicalHandsSettings.RecommendedSetting recommendedSetting;
        if (!PhysicalHandsSettings.Instance.recommendedSettings.TryGetValue(recommendedSettingKey, out recommendedSetting)) return;
        bool settingApplied = PhysicalHandsSettings.Instance.IsRecommendedSettingApplied(recommendedSettingKey);
        ...
        switch (column)
        {
            ...
            case Columns.Apply:
                ...
                {
                    PhysicalHandsSettings.Instance.ApplyRecommendedSetting(recommendedSettingKey);
                    Reload();
                    return;
                }
                break;
            ...
            case Columns.Ignore:
                ...
                if (recommendedSetting.ignored)
                {
                    if (GUI.Button(new Rect(cellRect.xMax - ((BUTTON_WIDTH / 2) * 2) - BUTTON_PADDING, cellRect.y + (BUTTON_HEIGHT / 4), BUTTON_WIDTH, BUTTON_HEIGHT), "Watch"))
                    {
                        ...
                        PhysicalHandsSettings.Instance.recommendedSettings[recommendedSettingKey] = recommendedSetting;
                        Reload();
                        return;
                    }
                }
