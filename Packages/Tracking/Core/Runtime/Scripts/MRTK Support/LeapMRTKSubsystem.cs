    ...
    public class LeapMRTKSubsystem : HandsSubsystem
    {
        ...
        private class LeapMRTKProvider : Provider, IHandsSubsystem
        {
            ...
            private static void RunAfterSceneLoad()
            {
                ...
                LeapProvider leapProvider = GameObject.FindAnyObjectByType<LeapProvider>();

                // If there is no leap provider in the scene
                if (leapProvider == null)
                {
                    Debug.Log("There are no LeapProviders in the scene, automatically assigning a LeapXRServiceProvider for use with Ultraleap Subsystem for MRTK");
                ...
                {
                    Debug.Log("Ultraleap Subsystem for MRTK is using the existing LeapProvider found in the current scene");
                }
